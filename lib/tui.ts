...
export function printCliHelp(): void {
  ...
  console.log(`    ${c.dim("$")} fell ${c.cyan("--list")} ${c.cyan("--json")}              ${c.dim("JSON output for machine consumption")}`)
  ...
  console.log(`    ${c.dim("$")} fell ${c.cyan("--status")} ${c.dim("<branch|path>")}     ${c.dim("Quick status of a single worktree")}`)
  console.log(`    ${c.dim("$")} fell ${c.cyan("--release")} ${c.dim("<branch|path>")}    ${c.dim("Release worktree (detach + delete branch)")}`)
  console.log(`    ${c.dim("$")} fell ${c.cyan("--clean")}                  ${c.dim("Auto-remove recyclable worktrees")}`)
  console.log(`    ${c.dim("$")} fell ${c.cyan("--clean")} ${c.cyan("--dry-run")}          ${c.dim("Preview what --clean would remove")}`)
  console.log(`    ${c.dim("$")} fell ${c.cyan("--sync")} ${c.dim("<branch|path>")}       ${c.dim("Fetch + rebase worktree onto origin/HEAD")}`)
